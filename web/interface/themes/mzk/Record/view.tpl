    ...
    <div class="yui-b first">
      <b class="btop"><b></b></b>
        ...
        <ul>
            ...
            {/if}
            <li id="saveLink"><a href="{$url}/Record/{$id|escape:"url"}/Save" class="fav" onClickDisabled="getLightbox('Record', 'Save', '{$id|escape}', null, '{translate text="Add to favorites"}'); return false;">{translate text="Add to favorites"}</a></li>
            {if !empty($addThis)}
        ...
        <div class="record">
          ...
          <div class="resultscroller">
            {if $previousRecord}<a href="{$url}/Record/{$previousRecord}" class="prev">&lsaquo; {translate text="Prev"}</a>{/if}
            #{$currentRecordPosition} {translate text='of'} {$resultTotal}
            {if $nextRecord}<a href="{$url}/Record/{$nextRecord}" class="next">{translate text="Next"} &rsaquo;</a>{/if}
          </div>
          {/if}
