      ...
      <dl class="narrowList navmenu narrow_begin">
        ...
        {else}
          {foreach from=$cluster.list item=thisFacet name="narrowLoop"}
            ...
            <dd id="more{$title}" class="more"><a href="#" onClick="moreFacets('{$title}'); return false;">{translate text='more'} ...</a></dd>
          </dl>
          <dl class="narrowList navmenu narrowGroupHidden" id="narrowGroupHidden_{$title}">
            ...
            {/if}
          {/foreach}
          {if $smarty.foreach.narrowLoop.total > 5}<dd class="less"><a href="#" onClick="lessFacets('{$title}'); return false;">{translate text='less'} ...</a></dd>{/if}
        {/if}
      </dl>
    {/foreach}
  {/if}
