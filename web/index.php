...
// enable profiler as soon as possible
if ($configArray['Profiler']['enabled']) {
   xhprof_enable();
}
...
} else if (isset($_GET['mylang'])) { // costumization for MZK
...
if (UserAccount::isLoggedIn() && $configArray['Authentication']['method'] == 'Shibboleth' 
    && $_SERVER[$configArray['Shibboleth']['username']] == null) {
    //logout();
    //$user = false;
} else if (!$user) {
    // Special case for Shibboleth:
    // Default case for all other authentication methods:
    $shibLoginNeeded = ($configArray['Authentication']['method'] == 'Shibboleth' && $module == 'MyResearch');
